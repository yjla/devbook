...
:::tip 形象记忆
原型像 **家传的工具箱**：每个对象出生时不必自带全套工具，而是手里攥着一根线 (`__proto__`)，牵到「爹」的工具箱 (原型对象)。自己没有的工具，顺着线去爹那儿借；爹也没有，再往爷爷那借——这根线一节节串起来，就是原型链。
:::
...
:::tip 形象记忆
这段演进像 **修一截水管**：第一版接通了水流 (方法能继承) 却漏水 (引用属性被所有实例共享)；第二版堵住漏点 (属性独立) 又断了水 (拿不到原型方法)；组合版水通也不漏，却多焊了一截废管 (父类构造调用两次)；直到寄生组合版，才做到又通、又不漏、又不浪费。
:::

...
function sumToN(n) {
  return new Promise((resolve) => {
    // 第一步：把「算到哪了」的状态放在闭包里，每片之间靠它续上
    let sum = 0;
    let i = 1;

    // 第二步：定义「算一片」——在 15ms 内尽量多算，超时就停手
    function chunk() {
      ...
      // 第三步：判断这一片结束后是算完了，还是只是时间到了
      if (i > n) {
        resolve(sum); // 全部算完，把结果交出去
      } else {
        // 还没算完 → 让出主线程，等浏览器空闲再跑下一片
...
`requestIdleCallback` 的回调自带一个 `deadline` 参数，`deadline.timeRemaining()` 返回**本次空闲期还剩多少毫秒**。直接拿它当循环条件，就不用自己记 `start` 了，而且更贴合「浏览器到底有多少空闲」的真实情况。形象例子：上一版是搬砖人自己掐表搬 15 秒；这一版是**工头告诉你「这条过道还能借你用 X 毫秒」**，你按工头报的剩余时间搬，过道忙就让你少搬、闲就让你多搬。
...
function sumToN(n) {
  return new Promise((resolve) => {
    ...
    // 第四步：启动第一片（注意这里也要用 requestIdleCallback 才能拿到 deadline）
    requestIdleCallback(chunk);
  });
}
```
...
const schedule =
  typeof requestIdleCallback !== 'undefined'
    ...
    : // 第三步：不支持（如老 Safari）就降级用 setTimeout 兜底
      (cb) => {
        setTimeout(cb, 0);
      };

// 之后统一用 schedule(chunk) 代替 requestIdleCallback(chunk)
```
...
> **大任务切成一片片，每片只跑十几毫秒就让出主线程，浏览器渲染完再续上下一片。进度靠闭包变量记住，算完才 `resolve`——这正是 React Fiber「可中断、可恢复」的内核。**

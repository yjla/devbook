...
- **减少回流与重绘**:批量改 DOM、读写分离(避免强制同步布局)、用 `class` 切换替代逐条改样式。参见 [回流与重绘](../javascript/browser/reflow-and-repaint.md)。
- **减少 DOM 数量与层级**。
...
RN 的瓶颈和 Web 不同,核心矛盾在 **JS 线程**、**渲染/通信** 和 **启动**。系统优化参见 [React Native 优化策略](./react-native-optimization.md),卡顿排查参见 [卡顿的原因和解决](./jank-causes-and-solutions.md)。

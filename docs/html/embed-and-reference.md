...
因为资源是元素不可缺的组成部分，浏览器**默认要等它加载、处理完**。`<script src>` 尤其典型：解析到它时会暂停 HTML 解析，先下载并执行脚本，再继续——这就是「阻塞」。所以传统建议把 `<script>` 放在 `<body>` 末尾，避免它卡住上面内容的渲染（更现代的做法见 [脚本加载时机](./script-loading.md)）。
